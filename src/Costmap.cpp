...
void Costmap::updateCells( vector<int8_t> &occupancy_grid_array ){
...
void Costmap::updateCostmap( vector<short int> &cmUpdates ){

	for(size_t i=0; i<cmUpdates.size(); i+=3){
		//x,y, val
		Point t(cmUpdates[i], cmUpdates[i+1]);
		cells.at<short>(t) = cmUpdates[i+2];
	}
}
