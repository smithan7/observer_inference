...
void Market::dissasembleTransmission(vector<float> &data){

	cerr << "data.size(): ";// << data.size() << endl;
	// turn the 1d vector into a standard market
	int transmissionLength = 13;
	for(int i=0; i<nAgents+1; i++){ // +1 is to account for observer
		cerr << "dissasembleTransmission: " << i << endl;
		if( data[i*transmissionLength+12] < times[i] && i != myIndex ){ // only update market if the info is new and not me
			cLocs[i].x = data[i*transmissionLength];
			cLocs[i].y = data[i*transmissionLength+1];

			gLocs[i].x = data[i*transmissionLength+2];
			gLocs[i].y = data[i*transmissionLength+3];
			exploreCosts[i] = data[i*transmissionLength+4];

			reportTimes[i] = data[i*transmissionLength+5];
			reportCosts[i] = data[i*transmissionLength+6];
			reportRequests[i] = data[i*transmissionLength+7];

			rLocs[i].x = data[i*transmissionLength+8];
			rLocs[i].y = data[i*transmissionLength+9];
			roles[i] = data[i*transmissionLength+10];
			mates[i] = data[i*transmissionLength+11];

			times[i] = data[i*transmissionLength+12]+1; // add 1 for my hop
		}
		cerr << "dissasembleTransmission2: " << i << endl;
		if( i == myIndex){ // if it is me check return and relay / sacrfice
			reportRequests[i] = data[i*transmissionLength+7]; // check for requests

			if(data[i*transmissionLength+10]){ // check if someone has made me a relay / sacrifice
				rLocs[i].x = data[i*transmissionLength+8];
				rLocs[i].y = data[i*transmissionLength+9];
				roles[i] = data[i*transmissionLength+10];
				mates[i] = data[i*transmissionLength+11];
		...
		cerr << "dissasembleTransmission3: " << i << endl;
	}
}
